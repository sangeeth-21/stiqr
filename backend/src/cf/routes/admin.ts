  ...
  app.post('/api/admin/shops', requireAdmin, async (c) => {
    try {
      ...
      await c.env.DB.batch([
        ...
        c.env.DB.prepare('INSERT INTO users (id, email, name, password, role, status, shopId, createdAt, updatedAt) VALUES (?,?,?,?,\'OWNER\',\'ACTIVE\',?,?,?)').bind(userId, ownerEmail, ownerName || name, hashed, shopId, now, now),
      ]);
