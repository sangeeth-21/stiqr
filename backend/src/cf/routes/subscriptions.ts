export function subscriptionRoutes(app: any) {
  app.get('/api/subscription', async (c) => {
    try {
      ...
      if (!tenantId) return c.json({ data: { plan: 'NONE', status: 'INACTIVE' } });
      const sub = await c.env.DB.prepare('SELECT * FROM subscriptions WHERE tenantId = ? ORDER BY createdAt DESC LIMIT 1').bind(tenantId).first();
  ...
  app.post('/api/subscription/renew', async (c) => {
    try {
      ...
      const existing = await c.env.DB.prepare('SELECT * FROM subscriptions WHERE tenantId = ? ORDER BY createdAt DESC LIMIT 1').bind(tenantId).first() as any;
      ...
      if (existing) {
        ...
        await c.env.DB.prepare('INSERT INTO subscriptions (id, tenantId, plan, status, startDate, endDate, autoRenew, createdAt, updatedAt) VALUES (?,?,?,?,?,?,0,?,?)').bind(crypto.randomUUID(), tenantId, plan || 'BASIC', 'ACTIVE', nowStr, endDate.toISOString(), nowStr, nowStr).run();
      }
  ...
  app.get('/api/subscription/history', async (c) => {
    try {
      ...
      const sub = await c.env.DB.prepare('SELECT id FROM subscriptions WHERE tenantId = ? ORDER BY createdAt DESC LIMIT 1').bind(tenantId).first() as any;
